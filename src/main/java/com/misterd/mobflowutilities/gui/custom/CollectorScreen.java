...
public class CollectorScreen extends AbstractContainerScreen<CollectorMenu> {
    ...
    private static final ResourceLocation XP_BAR_BG = ResourceLocation.fromNamespaceAndPath("mobflowutilities", "xp_bar");

    private static final ResourceLocation XP_BAR_PROGRESS = ResourceLocation.fromNamespaceAndPath("mobflowutilities", "xp_bar_progress");

    private static final WidgetSprites REDUCE_OFFSET_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "reduce_offset_btn"),
    ...
    private static final WidgetSprites INCREASE_OFFSET_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "increase_offset_btn"),
    ...
    private static final WidgetSprites TOGGLE_WIREFRAME_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "toggle_zone_wireframe_btn"),
    ...
    private static final WidgetSprites WITHDRAW_XP_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_xp_btn_hover"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_xp_btn_hover"));

    private static final WidgetSprites DEPOSIT_XP_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_xp_btn_hover"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_xp_btn_hover"));

    private static final WidgetSprites WITHDRAW_ALL_XP_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_all_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_all_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_all_xp_btn_hover"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "withdraw_all_xp_btn_hover"));

    private static final WidgetSprites DEPOSIT_ALL_XP_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_all_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_all_xp_btn"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_all_xp_btn_hover"),
            ResourceLocation.fromNamespaceAndPath("mobflowutilities", "deposit_all_xp_btn_hover"));

    private static final WidgetSprites RESET_OFFSET_SPRITES = new WidgetSprites(ResourceLocation.fromNamespaceAndPath("mobflowutilities", "collection_zone_offset_reset_btn"),
    ...
    private float getProgressToNextLevel(int totalXp) {
        int level = this.xpToLevel(totalXp);
        int xpForCurrentLevel = this.levelToXp(level);
        int xpNeededForNextLevel = this.getXpNeededForLevel(level);
        if (xpNeededForNextLevel <= 0) {
            return 0.0F;
        }
        float progress = (float) (totalXp - xpForCurrentLevel) / (float) xpNeededForNextLevel;
    ...
    private void resetAllOffsets() {
        ...
        PacketDistributor.sendToServer(new ConfigPacket(ConfigPacket.ConfigTarget.COLLECTOR_BLOCK, this.menu.blockEntity.getBlockPos(),
    ...
    private void withdrawXP() {
        ...
        PacketDistributor.sendToServer(new CollectorXpPacket(this.menu.blockEntity.getBlockPos(), CollectorXpPacket.XpAction.WITHDRAW_ONE_LEVEL, 0));
    }

    private void depositXP() {
        if (this.minecraft.player == null) return;
        if (this.minecraft.player.experienceLevel <= 0) return;
        PacketDistributor.sendToServer(new CollectorXpPacket(this.menu.blockEntity.getBlockPos(), CollectorXpPacket.XpAction.DEPOSIT_ONE_LEVEL, 0));
    }

    private void withdrawAllXP() {
        if (this.storedXP > 0) {
            PacketDistributor.sendToServer(new CollectorXpPacket(this.menu.blockEntity.getBlockPos(), CollectorXpPacket.XpAction.WITHDRAW, this.storedXP));
            this.storedXP = 0;
        }
    }
    ...
    private void depositAllXP() {
        ...
        if (playerXP > 0) {
            PacketDistributor.sendToServer(new CollectorXpPacket(this.menu.blockEntity.getBlockPos(), CollectorXpPacket.XpAction.DEPOSIT, playerXP));
    ...
    private void renderOffsetValues(GuiGraphics guiGraphics, int x, int y) {
        ...
        guiGraphics.drawString(this.font, (this.downUpOffset >= 0 ? "+" : "") + this.downUpOffset, (int) ((x + 194) / scale), (int) ((y + 71) / scale), 0xFF000000, false);
        guiGraphics.drawString(this.font, (this.northSouthOffset >= 0 ? "+" : "") + this.northSouthOffset, (int) ((x + 194) / scale), (int) ((y + 93) / scale), 0xFF000000, false);
        guiGraphics.drawString(this.font, (this.eastWestOffset >= 0 ? "+" : "") + this.eastWestOffset, (int) ((x + 194) / scale), (int) ((y + 115) / scale), 0xFF000000, false);
        poseStack.popPose();

        poseStack.pushPose();
        poseStack.scale(scale, scale, 1.0F);
        guiGraphics.drawString(this.font, Component.translatable("gui.mobflowutilities.collector.offset.down_up").getString(), (int) ((x + 182) / scale), (int) ((y + 60) / scale), 0xFF000000, false);
        guiGraphics.drawString(this.font, Component.translatable("gui.mobflowutilities.collector.offset.north_south").getString(), (int) ((x + 182) / scale), (int) ((y + 82) / scale), 0xFF000000, false);
        guiGraphics.drawString(this.font, Component.translatable("gui.mobflowutilities.collector.offset.east_west").getString(), (int) ((x + 182) / scale), (int) ((y + 104) / scale), 0xFF000000, false);
        poseStack.popPose();
    }
    ...
    private void renderXPCollectionToggle(GuiGraphics guiGraphics, int x, int y) {
        ...
        guiGraphics.blitSprite(toggleHandle, handleX, y + 111, 6, 10);
    }

    private void renderXPDisplay(GuiGraphics guiGraphics, int x, int y) {
        guiGraphics.blitSprite(XP_BAR_BG, x + 7, y + 137, 162, 5);

        int progress = (int) (this.getProgressToNextLevel(this.storedXP) * 162.0F);
        if (progress > 0) {
            guiGraphics.blitSprite(XP_BAR_PROGRESS, 162, 5, 0, 0, x + 7, y + 137, progress, 5);
        }
        ...
        guiGraphics.drawString(this.font, xpDisplayText, centeredX, (int) ((y + 122) / scale), 0xFF000000, false);
        poseStack.popPose();
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0 && mouseX >= x + 9 && mouseX <= x + 25 && mouseY >= y + 111 && mouseY <= y + 121) {
    ...
    private void renderCustomTooltips(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (mouseX >= x + 192 && mouseX <= x + 204 && mouseY >= y + 68 && mouseY <= y + 76) {
            guiGraphics.renderTooltip(this.font, Component.translatable("tooltip.mobflowutilities.collector.offset.down_up.value", this.downUpOffset), mouseX, mouseY);
            return;
        }
        if (mouseX >= x + 192 && mouseX <= x + 204 && mouseY >= y + 90 && mouseY <= y + 98) {
            guiGraphics.renderTooltip(this.font, Component.translatable("tooltip.mobflowutilities.collector.offset.north_south.value", this.northSouthOffset), mouseX, mouseY);
            return;
        }
        if (mouseX >= x + 192 && mouseX <= x + 204 && mouseY >= y + 112 && mouseY <= y + 120) {
            guiGraphics.renderTooltip(this.font, Component.translatable("tooltip.mobflowutilities.collector.offset.east_west.value", this.eastWestOffset), mouseX, mouseY);
        }
    }
